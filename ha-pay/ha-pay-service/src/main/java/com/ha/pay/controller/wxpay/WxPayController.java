...
import com.ha.pay.pojo.H5ScencInfo;
import com.ha.pay.properties.WxPayProperties;
...
public class WxPayController extends WxPayApiController {
	...
	public WxPayApiConfig getApiConfig() {
		notify_url = wxPayProperties.getDomain().concat("/wxpay/pay_notify");
		return WxPayApiConfig.New()
				.setAppId(wxPayProperties.getAppId())
				.setMchId(wxPayProperties.getMchId())
				.setPaternerKey(wxPayProperties.getPartnerKey())
	...
	@ResponseBody
	public String test() {
		return wxPayProperties.toString();
	}

	@RequestMapping("/getKey")
	@ResponseBody
	public String getKey() {
		return WxPayApi.getsignkey(wxPayProperties.getAppId(), wxPayProperties.getPartnerKey());
	}
	...
	@ResponseBody
	public AjaxResult aappPay(HttpServletRequest request,String openId) {
		String ip = IpKit.getRealIp(request);
		if (StrKit.isBlank(ip)) {
			ip = "127.0.0.1";
		}
		...
		Map<String, String> params = WxPayApiConfigKit
				...
				.setAttach("测试")
				.setBody("小程序支付测试")
				.setSpbillCreateIp(ip).setTotalFee("10")
	...
	public String oauth(@RequestParam("code") String code){
		try {
			...
			String url = "https://api.weixin.qq.com/sns/jscode2session?appid="
					+ "wx175aacee5b06c0c0" + "&secret=" + "f63935ef5f988c334f955ad3b399cfed"
	...
	public String transfers(HttpServletRequest request, @RequestParam("openId") String openId) {
		...
		params.put("mch_appid", wxPayProperties.getAppId());
		...
		params.put("sign", PaymentKit.createSign(params, wxPayProperties.getPartnerKey()));
		System.out.println("certPath>" + wxPayProperties.getCertPath());
		String url = this.getClass().getClassLoader().getResource(wxPayProperties.getCertPath()).getPath();
		// 提现
		String transfers = WxPayApi.transfers(params, url, wxPayProperties.getMchId());
	...
	public String transferInfo(@RequestParam("partner_trade_no") String partner_trade_no) {
		try {
			...
			return WxPayApi.getTransferInfo(params, wxPayProperties.getCertPath(), wxPayProperties.getMchId());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public String getPublicKey() {
		try {
			...
			String createSign = PaymentKit.createSign(params, wxPayProperties.getPartnerKey());
			params.put("sign", createSign);
			String publicKeyStr = WxPayApi.getPublicKey(params, wxPayProperties.getCertPath(), wxPayProperties.getMchId());
			return publicKeyStr;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String payBank() {
		try {
			...
			params.put("amount", "1");
			params.put("desc", "测试付款到银行卡");
			params.put("sign", PaymentKit.createSign(params, wxPayProperties.getPartnerKey()));
			String payBank = WxPayApi.payBank(params, wxPayProperties.getCertPath(), wxPayProperties.getMchId());
			return payBank;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String queryBank(@RequestParam("partner_trade_no") String partner_trade_no) {
		try {
			...
			params.put("nonce_str", System.currentTimeMillis() + "");
			params.put("sign", PaymentKit.createSign(params, wxPayProperties.getPartnerKey()));
			String queryBank = WxPayApi.queryBank(params, wxPayProperties.getCertPath(), wxPayProperties.getMchId());
			return queryBank;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String refund(@RequestParam("transactionId") String transactionId,
		...
		params.put("refund_fee", "10");
		params.put("sign", PaymentKit.createSign(params, wxPayProperties.getPartnerKey()));
		String url = this.getClass().getClassLoader().getResource(wxPayProperties.getCertPath()).getPath();
		String refund = WxPayApi.orderRefund(false, params,url, wxPayProperties.getMchId());
		//String refund = WxPayApi.orderRefund(false, params, wxPayProperties.getCertPath(), wxPayProperties.getMchId());
		System.out.println(refund);
		Map<String, String> returnParams = PaymentKit.xmlToMap(refund);
		System.out.println("returnParams:"+returnParams.toString());
		return refund;
	}
