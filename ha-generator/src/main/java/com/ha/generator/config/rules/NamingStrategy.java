package com.ha.generator.config.rules;

import com.ha.generator.common.OthersUtils;
import com.ha.generator.common.StringPool;
import com.ha.generator.common.StringUtils;
...
public enum NamingStrategy {
    ...
    public static String underlineToCamel(String name) {
        ...
        // 处理真正的驼峰片段
        Arrays.stream(camels).filter(camel -> !OthersUtils.isEmpty(camel)).forEach(camel -> {
            if (result.length() == 0) {
                // 第一个驼峰片段，全部字母都小写
                result.append(camel);
            } else {
    ...
    public static boolean isPrefixContained(String name, String... prefix) {
        if (null == prefix || OthersUtils.isEmpty(name)) {
            return false;
        }
